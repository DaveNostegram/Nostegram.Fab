namespace Nostegram.Fab.Infrastructure.Persistence.Seeding;

using Microsoft.EntityFrameworkCore;
using Nostegram.Fab.Domain;

public class FabSeedService : IFabSeedService
{
    private readonly FabDbContext _context;

    public FabSeedService(FabDbContext context)
    {
        _context = context;
    }
    private async Task SeedArtist(CancellationToken ct)
    {
        var existingArtistNames = await _context.Artists
            .Select(x => x.Name)
            .ToListAsync(ct);

        var existing = existingArtistNames
           .ToHashSet(StringComparer.OrdinalIgnoreCase);

        var artistsToAdd = _artistNames
            .Where(name => !existing.Contains(name))
            .Select(name => new Artist
            {
                Name = name
            })
            .ToList();

        if (artistsToAdd.Count == 0)
            return;

        await _context.Artists.AddRangeAsync(artistsToAdd, ct);
        await _context.SaveChangesAsync(ct);

    }
    private static readonly string[] _artistNames =
    [
        "Adolfo Navarro",
    "Agri Karuniawan",
    "Agus Setiawan",
    "Akifa Shahgeldleva",
    "Akifa Shakhgeldieva",
    "Aleksey Peregudov",
    "Alena Marchenko",
    "Alexander Gering",
    "Alexander Mokhov",
    "Alexander Nikolaev",
    "Alexander Pachin",
    "Alexander Staronosov",
    "Alexandra Malygina",
    "Alexey Peregrudov",
    "Alexis Susani",
    "Alief Rusdiatama",
    "Alif Zahrah",
    "Alifka Hammam",
    "Alifka Hammam Nugroho",
    "Alifka Hamman",
    "Alsu Nurieva",
    "Aluísio Cervelle",
    "Ambang Tegar Prakoso",
    "Amelia Tan",
    "Amirul HHF",
    "Amtepra",
    "Anastasia Alexandrova",
    "Anastasia Balakchina",
    "Anastasia Chernaya",
    "Anastasia Pronina",
    "Anastasiya Grintsova",
    "Andi Cahyo W",
    "Andrew Chou",
    "Andrey Potemkim",
    "Andrey Savchuk",
    "Andy Aslamov",
    "Angelina Tyshchik",
    "Anna Kharitonova",
    "Anna Kovalevskaya",
    "Anna Tanygina",
    "Anthony Avon",
    "Anton Bezrukov",
    "AOJI Maiko",
    "Arghawana Rim",
    "Arif Restu",
    "Arif Wijaya",
    "Arkoii",
    "Artem Maksimishin",
    "Artur Treffner",
    "Asep Ariyanto",
    "Askapoj T",
    "Asur Misoa",
    "Athiwut B.",
    "Audy Ravindra",
    "Ausonia",
    "Bastien Jez",
    "Billy Christian",
    "bimawithpencil",
    "Bo Setiaji",
    "Bramasta Aji",
    "Brian Adriel",
    "Brian Madya Narendra",
    "BWusagi",
    "Cacho Rubione",
    "Camille Alquier",
    "Carlos Cruchaga",
    "Carlos Jose Camus",
    "CGlas",
    "Chris Cold",
    "Christ Adiel Then",
    "Cindy Handoyo",
    "Cristián Huerta",
    "Daken",
    "Daniel Correia",
    "Daniel Jiménez",
    "Daniil Goncharov",
    "Daria Cherkashina",
    "Daria Khlebnikova",
    "David Ogilvie",
    "Diana Tanamas",
    "Dmitriy Semyonov",
    "Dmitry Isakevitch",
    "Dmitry Solodovnikov",
    "Dominik Mayer",
    "Dzulfikar Aliy",
    "Edward Chee",
    "Eilene Cherie",
    "Ekaterina Revazashvili",
    "Elena Bakulina",
    "Elena Danilova",
    "Elena Perelygina",
    "Eleonor Piteira",
    "Elina Akhiamova",
    "Elizaveta Alipatova",
    "Elizaveta Zaichikova",
    "Elya Nur Sri Ayu",
    "Emanuel Dias",
    "Emilis Emka",
    "Enmoire",
    "Eric Klug",
    "Erick Efata",
    "Eryk Szczygieł",
    "Esty Swandana",
    "Faizal Fikri",
    "Fajareka Setiawan",
    "Federico Musetti",
    "Fedor Barkhatov",
    "FeiKang",
    "Felicia Liang",
    "Firaz M. Rasyid",
    "Flora Silve",
    "Florian Herold",
    "Galih M",
    "Gorshkov Stanislav",
    "Grant Griffin",
    "Grégory Nunkovics",
    "Grigoriy Parshakov",
    "Harry Wyatt",
    "Hatori Kyoka",
    "Hendry Iwanaga",
    "Henrique Lindner",
    "Henry Leung",
    "Hilary Purnamasari",
    "Hoàng Lập",
    "Iain Miki",
    "Igor Heras",
    "Ilham Bahtiar",
    "Immanuela Crovius",
    "Ina Wong",
    "Inkognit",
    "Irene Francisco",
    "Irina Kononenko",
    "Irina Plotnikova",
    "Irina Si",
    "Ironical Ghosty",
    "Ismatulloh",
    "Isuardi Therianto",
    "J. Sebastián S.C.",
    "Jackson Tjota",
    "János Brumár",
    "Jason Kiantoro",
    "Jefrey Yonathan",
    "Jen Santos",
    "Jessada Sutthi",
    "Jessica Nguyen",
    "Jessketchin",
    "João G. Santos",
    "João Guilherme",
    "Joseph Qiu",
    "Joshua Raphael",
    "Jsraphael",
    "Junaidi Lim",
    "Kalashnikova N.",
    "Kanadekana",
    "Kate Fox",
    "Kelvin Jauwri",
    "Kevin Sidharta",
    "Khairul Sukmanudin",
    "Kimberly Pantoni",
    "King Fung Ng",
    "Ksenia Belova",
    "Kyxarie Peralta",
    "Le Vuong",
    "Leo Avero",
    "Leony Tobing",
    "Lius Lasahido",
    "Livia Prima",
    "Lizhe Liang",
    "M Fakhrur Rozzi",
    "M. Fakhrur Rozzi",
    "Mace Tan",
    "Madeline Boni",
    "Maerel Hibadita",
    "Marcellino Tan",
    "Marco González",
    "Marco Wulfr",
    "Marcus Reyno",
    "Maria Mishina",
    "Marina Lunina",
    "Mario Wibisono",
    "Mariusz Gandzel",
    "Mark Poole",
    "Masha Mishina",
    "Mateus Manhanini",
    "Mateusz Wiśniewski",
    "Max Kostin",
    "Maxim Nikiforov",
    "Michal Makowski",
    "Mihail Spil-Haufter",
    "Mike Dalzell",
    "Mikhail Babkin",
    "Milena Vasyukova",
    "Milivoj Ćeran",
    "MJ Fetesio",
    "Muhammad Fajri",
    "Murilo Amgarten",
    "Nadia Suryani",
    "Nailsen Ivanderlie",
    "Narendra B Adi",
    "Nathaniel Himawan",
    "Nikita Bondarev",
    "Nikko Wahyudi",
    "Nikolai Maslakov",
    "Nikolay Moskvin",
    "Nino Setiawan",
    "Nuttapong Thanawiwitaporn",
    "Olga Khariton",
    "Olga Krivolapova",
    "Olga Tereshenko",
    "Oliver Morit",
    "Orson CaptainSass",
    "Othon Nikolaidis",
    "Oxana Turlay",
    "Panji Bagus P",
    "Pascal Quidault",
    "Patchanee Treedet",
    "Pavel Chuvin",
    "Pavel Kondrashov",
    "Pavel Rtishchev (Klaher)",
    "Pavel Yurev",
    "Petrinda Wiyugo",
    "Peyeyo",
    "Philip Sue",
    "Phu Thieu",
    "Prisca Ardiana",
    "Rachel Alderson",
    "Ramza Ardyputra",
    "Raphael Masalimov",
    "Raphael Pinna",
    "Raynor Maharsi",
    "Regina Krivolapova",
    "Reinaldo Indrajaya",
    "Revazashvili E.",
    "Reza Afshar",
    "Reza Aswadananta",
    "Richard Elrassi",
    "Rio Sabda",
    "Riordan Delmiro",
    "Rodrigo Rizo",
    "romiy",
    "Saad Irfan",
    "Sam Yang",
    "Samuel Perin",
    "Sandeep Karunakaran",
    "SanSan",
    "Sariya Asavametha",
    "Satriasa",
    "Satrya Darmawan",
    "Sebastian Giacobino",
    "Sebastian Szmyd",
    "Septimius Ferdian",
    "Sergey Averkin",
    "Sergey Gurskiy",
    "Shen Fei",
    "Shiro Yayoi",
    "Silvia Meiliani",
    "Simon Dominic",
    "Simon Wong",
    "Soffani Soffa",
    "Sofia Akimova",
    "Sonia Sandoval",
    "Sooraj Babu",
    "soyameii",
    "Stanislav Sherbakov",
    "Steve Argyle",
    "Stormy Elia Fanggidae",
    "Surya P",
    "Surya Prasetya",
    "Tanapon Wachirakul",
    "Tatiana Trubnikova",
    "Thomas Ressuge",
    "Tian Thongjomroon",
    "Todor Hristov",
    "Tomasz Jedruszek",
    "Tomi Ong",
    "Tommy Suhartono",
    "Trung Tin Shinji",
    "Vadim Kalabukh",
    "Vathan Suwannaworn",
    "Vera Rudichenko",
    "Veto Zomer",
    "Victoria Belyaeva",
    "Viktoria Belyaeva",
    "Vincent Taslim",
    "Vito Febrianno",
    "Vito Vivodka",
    "Vlad Tashkinov",
    "Vlad Varlamov",
    "Vladimir Shatunov",
    "Widya Wang",
    "Wilson Nugraha",
    "Wirawan Pranoto",
    "Wisnu Tan",
    "YDZ",
    "Yen Lee",
    "Yeong-Hao Han",
    "Yiian Kho",
    "Yolanda Felicia",
    "Yosi Saputra",
    "Yugin Maffioli",
    "Yulia Litvinova",
    "Yulia Tarushko",
    "Yunior Susanto",
    "寿多浩 (Hiro Suda)"
    ];

}